...
void Graphic::GetFileImage (QString file)
{
    if (!image)
        delete (image);
    QImage *img = new QImage (file);                        //картинка
    image = myScene->addPixmap(QPixmap::fromImage(*img));   //добавляем пиксмап и получаем итем
    myScene->setSceneRect(0, 0, img->width(), img->height());   //размер сцены под картинку
}
...
QPixmap* Graphic::getNegative ()           //негатив
{
    ...
    if (image != nullptr)
    {
        newImage = *new QImage (image->pixmap().toImage());
